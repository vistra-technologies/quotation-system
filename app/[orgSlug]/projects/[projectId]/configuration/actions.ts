...
export type UpdateSelectionState = { error: string | null };
 ...
 *
 * Stage 17: thin marshaler — parses FormData, delegates to
 * PATCH /api/v1/orgs/[orgSlug]/selections/[id] via internalFetch.
 * componentTypeId is not patchable — locked after creation.
 *
 * Uses the useActionState signature so the client form can surface errors.
 */
export async function updateSelection(
  ...
  if (!selectionId || !label) {
    return { error: "Selection ID and label are required." };
  }
