/**
 * Inquiries list route — loading UI (Suspense boundary).
 *
 * Shown while the server renders the inquiries list (session + DB round-trips).
 * Eliminates the blank-screen pause on navigation by streaming the shell immediately.
 *
 * Stage 12 Batch 7c: updated to reflect the new toolbar (date filter, search,
 * toggle) + reduced row height + pagination footer.
 */
      ...
      <div className="rounded-md border border-border bg-bg-card">
        <div className="px-4">
          ...
          <div className="flex gap-4 border-b border-border py-[10px]">
            {[180, 140, 100, 80, 100, 100].map((w, i) => (
