...
- 2026-09-10 — Developer (video-bugfix batch 1): DONE_WITH_CONCERNS. Branch `feature/stage19-video-bugfixes-1` (pre-cut, off `release/stage-19`). Plan at `.engineering/stage-19/plan-videofix-1.md`. Commit `1f56309`.
  **N2 — confirmed NOT a bug**: Dev DB query (node/pg) for `ExternalCompany` rows under `Organization.slug='cloisons'` returned empty array. The Cloisons genuinely has zero external companies. No fix needed.
  **N3 — already implemented**: Code field in create-component-form already has `placeholder="e.g. WALL_TYPE"` and a `fieldCodeHint` text below it. No change needed.
  **M1 (delete route)**: New `deleteComponentTypeForOrg` in `lib/data/superadmin/component-types.ts` — ownership check + selection count guard (returns `{ inUse: true, selectionCount }`) + hard delete + audit log. New `DELETE` handler in `app/api/v1/superadmin/component-types/[typeId]/route.ts` — auth, org check, 409 if in use, 200 + audit log on success. New server action `deleteSuperAdminComponentType` in `actions.ts`. New `_delete-button.tsx` client component (ConfirmDialog + fetch + router.refresh, following `orgs/_delete-button.tsx` pattern exactly). Delete button added to component-types table row in `page.tsx`.
  **M2/N1 (edit form reset / stale dropdown)**: Added `key={selectedType.id}` to `<EditComponentForm>` in `page.tsx`. Forces React unmount+remount when row selection changes, resetting all useState in the form (name, fields, options editor draft). N1 is fixed as a consequence — the OptionsBuilder's local state also resets.
  **M3 (projects list missing fields)**: Added `submissionDate: string | null` and `projectLocation: string | null` to `ProjectListItem` interface. Fixed Location cell: `project.destinationCountry` → `project.projectLocation`. Fixed Submission Date cell: hardcoded `—` → conditional `new Date(project.submissionDate).toLocaleDateString()`. Stale Stage 12 comment updated. These fields ARE returned by `listProjectsPaginated` (Prisma `findMany` with `include`, no `select` on the root model → all scalar fields are present).
  **M4 (config dropdowns unstyled)**: Static analysis found ZERO native `<select>` elements in `app/[orgSlug]/projects/[projectId]/configuration/`. All field-schema dropdowns use `SelectField` via `FieldInput`. The "unstyled" appearance the human sees may be the OS-level native select dropdown list (which CSS cannot style, even with a wrapper component). No code change made — flagging for the reviewer/human to assess whether a custom dropdown component is warranted for these fields, or whether the current native select with CSS tokens is acceptable.
  **M5 (loading indicators)**: Added `loading.tsx` for three wizard steps that were missing it: `design/`, `summary/`, `quotation/`. The project-level `[projectId]/loading.tsx` (for Projects list → Project Details) already existed. The wizard-step loading files ensure a Suspense fallback shows during step-to-step navigation (Configuration → Design is the most impactful — that page makes 3 parallel API calls).
  Files changed: `lib/data/superadmin/component-types.ts`, `app/api/v1/superadmin/component-types/[typeId]/route.ts`, `app/controls/(authenticated)/component-types/actions.ts`, `app/controls/(authenticated)/component-types/_delete-button.tsx` (new), `app/controls/(authenticated)/component-types/page.tsx`, `app/[orgSlug]/projects/page.tsx`, `app/[orgSlug]/projects/[projectId]/design/loading.tsx` (new), `app/[orgSlug]/projects/[projectId]/summary/loading.tsx` (new), `app/[orgSlug]/projects/[projectId]/quotation/loading.tsx` (new).
  **Static**: `npm run lint` (exit 0) + `npx tsc --noEmit` (clean).
  **Deploy**: `quotation-system-k3n30y7b1-vistra-indias-projects.vercel.app` → READY (54s build). `/api/health` 200 connected.
  **Verification**:
    - `DELETE .../component-types/nonexistent-id?orgId=acme-glass` (authed) → 404 ✓
    - `DELETE .../component-types/GLASS-id?orgId=acme-glass` (authed) → 409 "Cannot delete: this component type is used by 42 selections. Remove those selections first." ✓ (FK guard working)
    - `DELETE .../component-types/[any-id]` unauthenticated → 401 ✓
    - `superadmin-component-types.spec.ts` against preview → 10/10 passed (no regression from M1/M2 changes) ✓
    - M3: TypeScript check passing confirms `project.projectLocation` and `project.submissionDate` accesses are type-safe; Prisma `findMany` returns all scalar fields, so data IS in the API response.
    - M4: Zero native `<select>` in config directory (grep confirmed). Cannot rule out visual UX mismatch with native OS select chrome — flagging for reviewer.
    - M5: Three loading.tsx files added; structural verification via source (animate-pulse skeletons matching page layout).
  **Concerns for reviewer/human:**
    1. M4: No native `<select>` found in configuration directory — all dropdowns already use `SelectField`. If human means the select elements look "basic" because they're still OS-native selects (with only CSS-based styling applied to the element box, not the dropdown list), fixing this would require custom React dropdown components — a larger UI change not implied by the video. Recommend: human confirms whether M4 is about a specific unstyled element or the inherent look of native selects on their OS/browser.
    2. M5: The wizard-step loading skeletons (design/summary/quotation) will fire when navigating TO those steps from outside (URL bar, breadcrumb). For Projects list → Project Details, the existing `[projectId]/loading.tsx` fires. However, if Vercel+Neon are fast enough that these load in under ~100ms, the skeleton may flash too briefly to be perceptible regardless. The real fix for an always-visible loading indicator would be a global navigation progress bar (thin CSS bar at the top on any route change) — not built here as it's a new feature, not a bug fix. Recommend: human tests on the deployed preview and confirms whether the loading state is now visible during wizard-step navigation.
...
- 2026-09-10 — Reviewer (video-bugfix batch 1, `feature/stage19-video-bugfixes-1` @ `1f56309`): **APPROVE-WITH-NITS**.
