...
**Outcome:** DONE. Correction from human: no cross-org capability exists (the original build's D2 navigated
to another org's `/inquiries?scope=all` — no role in this system has cross-org access, so this could never
have worked). The org-switcher was replaced with an external-company filter dropdown (within-org filtering
only), per the human's clarification: "dropdown to select external companies for whom an Inquiry is made
...
- `npm run lint` → 0 errors ✓
- `npx tsc --noEmit` → 0 errors ✓
- Pushed to `feature/batch7c-inquiries-list-pattern` at `ab79dea`.

### Batch 7c — reviewer: fix verification (2026-07-31)

**Verdict:** APPROVE-WITH-NITS · 0 CRITICAL · 0 IMPORTANT · 3 MINOR

**Report:** [`.engineering/stage-12/review-batch7c.md`](./review-batch7c.md)

Tenancy confirmed solid: `organizationId` scope unconditional, `externalCompanyId` filter additive and
gated on `session.externalCompanyId === null` (internal-only), no residual cross-org code (props, nav)
remains. MINOR-1 (worth taking): `handleScopeChange` doesn't clear `externalCompanyId` when switching to
"mine" scope, and the DAL still applies it silently in that case — a company-only filter also isn't
reflected in the empty-state message. MINOR-2 (developer's discretion): debounce timer not cleared on
