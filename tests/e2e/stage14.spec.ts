...
test("destinationCountry: INDIA company shown as India on inquiry detail", async ({ page }) => {
  await signIn(page, "admin");
  const coName = "S14-India-Co-" + Date.now();
  await createExternalCompany(page, coName, "INDIA");

  await page.goto("/acme-glass/inquiries/new");
  await page.waitForURL(/inquiries\/new/, { timeout: 20_000 });
  await selectCompany(page, coName);
  const inqName = "S14-Inq-India-" + Date.now();
  await fillInquiryRequiredFields(page, inqName, "22AAAAA0000A1Z5");
  ...
  await expect(page.getByText("India", { exact: true }).first()).toBeVisible({ timeout: 10_000 });
});

test("destinationCountry: UAE company shown as UAE on inquiry detail", async ({ page }) => {
  await signIn(page, "admin");
  const coName = "S14-UAE-Co-" + Date.now();
  await createExternalCompany(page, coName, "UAE");

  await page.goto("/acme-glass/inquiries/new");
  await page.waitForURL(/inquiries\/new/, { timeout: 20_000 });
  await selectCompany(page, coName);
  const inqName = "S14-Inq-UAE-" + Date.now();
  await fillInquiryRequiredFields(page, inqName);
  ...
  await expect(page.getByText("UAE", { exact: true }).first()).toBeVisible({ timeout: 10_000 });
});
...
test("convertInquiryToProject: all 15 sentinel fields appear on resulting project", async ({ page }) => {
  ...
  await page.locator("input[name='projectBudget']").fill("BUDGET_SENTINEL_XYZ");
  ...
  await page.locator("input[name='mainContractorName']").fill("MAIN_CONTRACTOR_SENTINEL");
  await page.locator("input[name='interiorContractorName']").fill("INT_CONTRACTOR_SENTINEL");
  await page.locator("input[name='mainConsultantName']").fill("MAIN_CONSULTANT_SENTINEL");
  await page.locator("input[name='interiorConsultantName']").fill("INT_CONSULTANT_SENTINEL");
  await page.locator("input[name='endClientName']").fill("ENDCLIENT_NAME_SENTINEL");
  ...
  await page.locator("input[name='endClientCity']").fill("CITY_SENTINEL");
  await page.locator("input[name='endClientState']").fill("STATE_SENTINEL");
  ...
  await expect(page.getByText("BUDGET_SENTINEL_XYZ")).toBeVisible({ timeout: 5_000 });
  await expect(page.getByText("MAIN_CONTRACTOR_SENTINEL")).toBeVisible({ timeout: 5_000 });
  await expect(page.getByText("INT_CONTRACTOR_SENTINEL")).toBeVisible({ timeout: 5_000 });
  await expect(page.getByText("MAIN_CONSULTANT_SENTINEL")).toBeVisible({ timeout: 5_000 });
  await expect(page.getByText("INT_CONSULTANT_SENTINEL")).toBeVisible({ timeout: 5_000 });
  await expect(page.getByText("ENDCLIENT_NAME_SENTINEL")).toBeVisible({ timeout: 5_000 });
  ...
  await expect(page.getByText("CITY_SENTINEL")).toBeVisible({ timeout: 5_000 });
  await expect(page.getByText("STATE_SENTINEL")).toBeVisible({ timeout: 5_000 });
